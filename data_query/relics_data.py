...
class Relic:
    ...
    def set_bonus(self) -> Generator[str | None, None, None]:
        ...
        for data in skill_data:
            ...
            if desc and params and set_num and name is None: return
    ...
    def sub_stat(self) -> Generator[dict, None, None] | None:
        ...
        for data in pieces_data:
            ...
            for key in rarity_data.keys():
                ...
                if sub_stats is None: return
                for sub_stat in sub_stats:
                    sub_stat_name: str | None = sub_stat.get("propertyName")
                    sub_stat_propertyicon: str = sub_stat.get(
                        "propertyIconPath"
                    )
                    sub_stat_ispercent: bool = sub_stat.get(
                        "isPercent"
                    )
                    base_value: float | int = sub_stat.get(
                        "baseValue"
                    )
                    level_add: float | int = sub_stat.get("levelAdd")
                    step_value: float | int = sub_stat.get(
                        "stepValue"
                    )
